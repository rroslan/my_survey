defmodule MySurveyWeb.UserAuthTest do
  use MySurveyWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias MySurvey.Accounts
  alias MySurvey.Accounts.Scope
  alias MySurveyWeb.UserAuth

  import MySurvey.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, MySurveyWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      MySurveyWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        MySurveyWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
